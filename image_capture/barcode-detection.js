...
const BARCODE_DETECT_INTERVAL_MS = 600;
...
function prepareDetectionCanvas(sourceCanvas) {
  detectCanvas.width = sourceCanvas.width;
  detectCanvas.height = sourceCanvas.height;
  detectCtx.imageSmoothingEnabled = false;
  detectCtx.drawImage(sourceCanvas, 0, 0);
  return detectCanvas;
}
...
async function detectBarcodeFromCanvas(sourceCanvas, settings) {
  const canvas = prepareDetectionCanvas(sourceCanvas);
  ...
  if (decoder === 'barcodeDetector') {
    return detectWithNativeApi(canvas, formatConfig);
  }
  ...
  try {
    ...
    if (decoder === 'zxingImage') {
      return detectWithZxingImageElement(canvas, zxing, hints, formatConfig);
    }
  } catch {
    return null;
  }
